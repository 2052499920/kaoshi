...
export default {
    ...
    _gywm() {
      ...
      if (this.$route.query.id) {
        obj = {
          ...
          title: this.title,
          gywm_id: this.gywm_id
        };
      } else {
        obj = { html: this.gywm_html, title: this.title, add: "add", gywm_id: this.gywm_id };
      }
      axios.post("/gywm", obj).then((res) => {
        this.$router.back(-1);
      });
    },
  },
  mounted() {
    ...
    axios
      .post("/gywm", { id: this.$route.query.id })
      .then((res) => {
        editor1.txt.html(res.data.data[0].html);
        this.title = res.data.data[0].title;
        this.gywm_id = res.data.data[0].gywm_id;
      });
