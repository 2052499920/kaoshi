...
export default {
  ...
  methods: {
    _gywm() {
      axios
        .post("/type3", { jiancha_state: this.$route.query.state, html: this.gywm_html, edit_html_id: this.$route.query.id })
        .then((res) => {
            this.$router.back(-1)
        });
    },
  ...
  mounted() {
    ...
    axios.post("/type3", { id: this.$route.query.id }).then((res) => {
      if(this.$route.query.state == 0) {
          editor1.txt.html(res.data.data[0].jcq);
      }
      if(this.$route.query.state == 1) {
          editor1.txt.html(res.data.data[0].jcz);
      }
      if(this.$route.query.state == 2) {
          editor1.txt.html(res.data.data[0].jch);
      }
    });
